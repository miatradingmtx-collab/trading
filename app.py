...
def actualizar_excel_local(alert: TradeAlert):
    """
    Registra automáticamente el trade en un archivo Excel local (Bitacora de entradas 2025.xlsx o Bitacora.xlsx).
    Mapea de forma inteligente las columnas según las cabeceras existentes en la primera fila.
    """
    try:
        import openpyxl
        from openpyxl import load_workbook
        
        # Determinar qué archivo Excel usar
        archivo = None
        rutas_posibles = [
            "Bitacora de entradas 2025.xlsx",
            "Bitacora.xlsx",
            "c:/Users/ecybe/OneDrive/Documentos/Trading/Bitacora de entradas 2025.xlsx",
            "c:/Users/ecybe/OneDrive/Documentos/Trading/Bitacora.xlsx"
        ]
        ...
        if not archivo:
            print("| EXCEL WARNING | No se encontró ningún archivo de Bitacora Excel. Omitiendo registro local.")
            return False
            
        wb = load_workbook(archivo)
        ws = wb.active # Abre la hoja activa
        print(f"| EXCEL | Abriendo hoja '{ws.title}' del archivo '{archivo}'...")
        
        # Mapear columnas según los encabezados de la fila 1
        headers = [str(cell.value).strip().lower() for cell in ws[1] if cell.value is not None]
        
        # Valores a insertar
        fecha_str = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        
        # Inicializar una fila vacía con el mismo tamaño que las columnas detectadas, o mínimo 8
        max_col = max(len(headers), 8)
        nueva_fila = [None] * max_col
        
        # Buscar mapeo dinámico
        mapeado = False
        for idx, h in enumerate(headers):
            if h in ["fecha", "date", "fecha de entrada", "fecha hora", "datetime", "fecha y hora"]:
                nueva_fila[idx] = fecha_str
                mapeado = True
            elif h in ["activo", "ticker", "instrumento", "par", "symbol", "símbolo", "simbolo"]:
                nueva_fila[idx] = alert.activo
                mapeado = True
            elif h in ["acción", "accion", "dirección", "direccion", "tipo", "action", "side", "compra/venta"]:
                nueva_fila[idx] = alert.accion
                mapeado = True
            elif h in ["precio", "precio de entrada", "precio entrada", "entry", "precio_entrada"]:
                nueva_fila[idx] = alert.precio
                mapeado = True
            elif h in ["stop loss", "sl", "stop_loss", "stop"]:
                nueva_fila[idx] = alert.stop_loss
                mapeado = True
            elif h in ["take profit", "tp", "take_profit", "profit target"]:
                nueva_fila[idx] = alert.take_profit
                mapeado = True
            elif h in ["estrategia", "strategy", "setup", "sistema"]:
                nueva_fila[idx] = alert.estrategia
                mapeado = True
            elif h in ["pnl", "resultado", "profit", "loss", "ganancia", "pérdida", "p&l"]:
                nueva_fila[idx] = alert.pnl
                mapeado = True
            elif h in ["ticket", "id", "orden", "operación", "operacion", "id_ticket", "ticket_id"]:
                nueva_fila[idx] = alert.ticket
                mapeado = True
                
        # Si no se encontró ningún encabezado conocido, agregar en el orden estándar al final de la hoja
        if not mapeado or all(v is None for v in nueva_fila):
            print("| EXCEL | No se detectaron encabezados conocidos. Escribiendo en orden estándar.")
            nueva_fila = [
                fecha_str,
                alert.activo,
                alert.accion,
            ...
            # Si la hoja está completamente vacía, agregar encabezados
            if ws.max_row == 1 and all(cell.value is None for cell in ws[1]):
                ws.append(["Fecha", "Activo", "Acción", "Precio Entrada", "Stop Loss", "Take Profit", "Estrategia", "PnL", "Ticket"])
                
        ws.append(nueva_fila)
        wb.save(archivo)
        print(f"| EXCEL SUCCESS | Trade de {alert.activo} guardado con éxito en {archivo}")
        return True
    except Exception as e:
...
def actualizar_aprendizaje_mia(activo: str, pnl: float):
    ...
    try:
        ...
        if racha >= 2 or (apoyo["trades_totales"] >= 3 and wr >= 60.0):
            apoyo["sentimiento_acumulado"] = "BULLISH"
            ...
            apoyo["sentimiento_acumulado"] = "BEARISH"
